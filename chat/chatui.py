import sys
import os
from chatcommon import ChatMessage


def print_chat_message(message: ChatMessage):
    match message.type:
        case "chat":
            print_message(f"{message.nickname}: {message.message}")
        case "join":
            print_message(f"*** {message.nickname} has just joined!")
        case "leave":
            print_message(f"*** {message.nickname} has just left!")
        case _:
            print_message(f"unkown mesage type: {message.type}")
