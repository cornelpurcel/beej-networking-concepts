...
def ipv4_to_value(ipv4_addr: str) -> int:
    ...
    return reduce(
        ...
        (
            ...
            for index, number in enumerate(ipv4_addr.split("."))
        ),
    )
...
def value_to_ipv4(addr: int) -> str:
    ...
    return ".".join(str(addr >> ((3 - index) * 8) & 0xFF) for index in range(4))
...
def ips_same_subnet(ip1: str, ip2: str, slash: str) -> bool:
    ...
    ip1_network = ipv4_to_value(ip1) & get_subnet_mask_value(slash)
    ip2_network = ipv4_to_value(ip2) & get_subnet_mask_value(slash)
...
def find_router_for_ip(routers: dict, ip: str):
    ...
    for router_network, router_info in routers.items():
        netmask = router_info["netmask"]

        if ips_same_subnet(ip, router_network, netmask):
